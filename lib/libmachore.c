...
void parse_section64(struct arch_analysis *arch_analysis, uint8_t *buffer,
                     struct section_64 *sect, char *segment_name) {
  PARSE_SECTION(arch_analysis, buffer, sect, segment_name);
}
...
void parse_text_segment64(struct arch_analysis *arch_analysis, uint8_t *buffer,
  ...
  for (uint32_t index = 0; index < seg->nsects; index++) {
    if (strcmp(sect->sectname, "__cstring") == 0 ||
      ...
      parse_section64(arch_analysis, buffer, sect, "__TEXT");
    }
    sect++;
  }
}

void parse_section(struct arch_analysis *arch_analysis, uint8_t *buffer,
